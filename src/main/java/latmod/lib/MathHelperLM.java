...
public class MathHelperLM {
    ...
    static {
        ...
        for (int i = 0; i < SIN_TABLE_SIZE; i++) SIN_TABLE[i] = Math.sin(i * ds);
    }
    ...
    public static int[] flip(int[] i) {
        ...
        for (int j = 0; j < i.length; j++) ai[j] = i[(i.length - 1) - j];
        return ai;
    }

    public static int[] getAllInts(int min, int size) {
        int[] ai = new int[size];
        for (int i = 0; i < size; i++) ai[i] = min + i;
        return ai;
    }
