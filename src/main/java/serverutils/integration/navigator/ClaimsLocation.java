...
public class ClaimsLocation implements IWaypointAndLocationProvider {
    ...
    public ClaimsLocation(long chunkCoord, ClientClaimedChunks.ChunkData data) {
        int chunkX = CoordinatePacker.unpackX(chunkCoord);
        int chunkZ = CoordinatePacker.unpackZ(chunkCoord);
        blockX = Util.coordChunkToBlock(chunkX);
        blockZ = Util.coordChunkToBlock(chunkZ);
        dimensionId = CoordinatePacker.unpackY(chunkCoord);
        chunkData = data;
    }
