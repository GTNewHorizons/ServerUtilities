...
public interface ServerUtilitiesLibGameRules {
	...
	static boolean canCreateTeam(World world) {
		return !ServerUtilitiesLibConfig.teams.disable_teams && !world.getGameRules().getGameRuleBooleanValue(DISABLE_TEAM_CREATION);
	}

	static boolean canJoinTeam(World world) {
		return !ServerUtilitiesLibConfig.teams.disable_teams && !world.getGameRules().getGameRuleBooleanValue(DISABLE_TEAM_JOINING);
	}
}