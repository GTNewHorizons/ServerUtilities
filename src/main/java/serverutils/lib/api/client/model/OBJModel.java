...
public class OBJModel {
    ...
    public static OBJModel load(ResourceLocation rl) {
        try {
            return OBJModel.load(OBJModel.class.getResourceAsStream(ServerUtilitiesLib.getPath(rl)));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
            ...
            while ((s = br.readLine()) != null) {
                if (s.length() > 0 && s.charAt(0) != '#') {
                    ...
                    if (s3[0].equals("o")) {
                        ...
                        m.current = g;
                    } else if (s3[0].equals("g")) {} else if (s3[0].equals("s")) {} else
                        if (s3[0].equals("mtllib")) {} else if (s3[0].equals("usemtl")) {} else if (s3[0].equals("v")) {
    ...
    public void renderAll() {
        for (int i = 0; i < groups.size(); i++) groups.get(i).render();
    }
    ...
    public void renderAllExcept(int... index) {
        for (int i = 0; i < groups.size(); i++) {
            for (int j = 0; j < index.length; j++) if (i == index[j]) continue;
            groups.get(i).render();
        }
    }
