...
public class ClientNotifications {
    ...
    public static class Temp extends FinalIDObject {
        ...
        private Temp(Notification n) {
            ...
            width = 20 + Math.max(
                    ServerUtilitiesLibraryClient.mc.fontRenderer.getStringWidth(title),
                    ServerUtilitiesLibraryClient.mc.fontRenderer.getStringWidth(desc));
        ...
        public boolean render() {
            ...
            if (time > 0L) {
                ...
                GlStateManager.translate(ServerUtilitiesLibraryClient.displayW - width, -d1 * 36D, 0F);

                GlStateManager.disableTexture2D();
                GlStateManager.disableLighting();
                ServerUtilitiesLibraryClient.setGLColor(notification.color, 230);
                GuiLM.drawBlankRect(0D, 0D, 0D, ServerUtilitiesLibraryClient.displayW, 32D);
                GlStateManager.enableTexture2D();
                GlStateManager.color(1F, 1F, 1F, 1F);

                int w = notification.item == null ? 10 : 30;

                FontRenderer font = ServerUtilitiesLibraryClient.mc.fontRenderer;
                ...
                if (notification.item != null) {
                    // ServerUtilitiesLibraryClient.renderGuiItem(notification.item,
                    // ServerUtilitiesLibraryClient.mc.getRenderItem(), font, 8, 8);
                }
