...
public class GuiToast extends Gui {
    ...
    public <T extends IToast> T getToast(Class<? extends T> p_192990_1_, Object p_192990_2_) {
        for (ToastInstance<?> toastinstance : this.visible) {
            if (toastinstance != null && p_192990_1_.isAssignableFrom(toastinstance.getToast().getClass())
                    && toastinstance.getToast().getType().equals(p_192990_2_)) {
    ...
    class ToastInstance<T extends IToast> {
        ...
        private float getVisibility(long p_193686_1_) {
            float f = MathHelper.clamp_float((float) (p_193686_1_ - this.animationTime) / 600.0F, 0.0F, 1.0F);
            f = f * f;
