...
import serverutils.lib.lib.util.misc.EnumPrivacyLevel;
	...
	void onLoggedIn(EntityPlayerMP player, Universe universe, boolean firstLogin) {
		...
		if (!hideTeamNotification() && !hasTeam()) {
			...
			b1.getChatStyle().setChatHoverEvent(
					new HoverEvent(HoverEvent.Action.SHOW_TEXT, ServerUtilitiesLib.lang(player, "sidebar_button.serverlib.my_team")));
