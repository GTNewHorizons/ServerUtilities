...
public class MessageMyTeamGui extends MessageToServer {
    ...
    public void onMessage(EntityPlayerMP player) {
        if (!ServerUtilitiesLibGameRules.canCreateTeam(player.worldObj)
                && !ServerUtilitiesLibGameRules.canJoinTeam(player.worldObj)) {
            ServerUtilitiesLibAPI.sendCloseGuiPacket(player);
        } else {
            ForgePlayer p = Universe.get().getPlayer(player);
            (p.hasTeam() ? new MessageMyTeamGuiResponse(p)
                    : new MessageSelectTeamGui(p, ServerUtilitiesLibGameRules.canCreateTeam(player.worldObj)))
                            .sendTo(player);
        }
    }
}
