...
import serverutils.lib.client.teamsgui.GuiManageAllies;
import serverutils.lib.client.teamsgui.GuiManageEnemies;
import serverutils.lib.client.teamsgui.GuiManageMembers;
import serverutils.lib.client.teamsgui.GuiManageModerators;
import serverutils.lib.client.teamsgui.GuiTransferOwnership;
...
public class MessageMyTeamPlayerList extends MessageToClient {
    ...
    public void onMessage() {
        if (id.equals(ServerUtilitiesLibTeamGuiActions.MEMBERS.getId())) {
            new GuiManageMembers(entries).openGui();
        } else if (id.equals(ServerUtilitiesLibTeamGuiActions.ALLIES.getId())) {
            new GuiManageAllies(entries).openGui();
        } else if (id.equals(ServerUtilitiesLibTeamGuiActions.MODERATORS.getId())) {
            new GuiManageModerators(entries).openGui();
        } else if (id.equals(ServerUtilitiesLibTeamGuiActions.ENEMIES.getId())) {
            new GuiManageEnemies(entries).openGui();
        } else if (id.equals(ServerUtilitiesLibTeamGuiActions.TRANSFER_OWNERSHIP.getId())) {
