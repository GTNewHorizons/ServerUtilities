...
public class CmdStatus extends CmdBase
{
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException
	{
		...
		{
			throw ServerLib.error(sender, "serverlib.lang.team.permission.owner");
		}
		else if (!p.team.isModerator(p))
		{
			throw new CommandException("commands.generic.permission");
		...
		if (status.canBeSet())
		{
			...
			sender.addChatMessage(ServerLib.lang(sender, "commands.team.status.set", p1.getDisplayName(), EnumTeamStatus.NAME_MAP.getDisplayName(sender, status)));
		}
		else
		{
			sender.addChatMessage(ServerLib.lang(sender, "commands.team.status.cant_set"));
		}
	}
}
