...
import serverutils.serverlib.lib.util.INBTSerializable;
...
import serverutils.serverlib.lib.util.permission.PermissionAPI;
import serverutils.serverlib.lib.util.permission.context.IContext;
import serverutils.serverlib.lib.util.permission.context.PlayerContext;
import serverutils.serverlib.lib.util.permission.context.WorldContext;
...
public class ForgePlayer implements INBTSerializable<NBTTagCompound>, Comparable<ForgePlayer>, IConfigCallback {
	...
	void onLoggedIn(EntityPlayerMP player, Universe universe, boolean firstLogin) {
		...
		if (firstLogin && (ServerLibConfig.teams.disable_teams
				|| (player.mcServer.isSinglePlayer() ? ServerLibConfig.teams.autocreate_sp
				: ServerLibConfig.teams.autocreate_mp))) {
		...
		if (!hideTeamNotification() && !hasTeam()) {
			...
			b1.getChatStyle().setChatClickEvent(
					new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/serverlib_simulate_button custom:serverlib:my_team_gui"));
			b1.getChatStyle().setChatHoverEvent(
					new HoverEvent(HoverEvent.Action.SHOW_TEXT, ServerLib.lang(player, "sidebar_button.serverlib.my_team")));
			...
			b2.getChatStyle().setChatClickEvent(
					new ClickEvent(
							...
							"/my_settings " + ServerLib.MOD_ID + ".hide_team_notification toggle"));
			b2.getChatStyle().setChatHoverEvent(
					new HoverEvent(
							HoverEvent.Action.SHOW_TEXT,
							ServerLib.lang(player, "serverlib.lang.team.notification.hide")));
			player.addChatMessage(ServerLib.lang(player, "serverlib.lang.team.notification", b1, b2));
		}
