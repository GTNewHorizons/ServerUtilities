...
import serverutils.serverlib.events.team.ForgeTeamConfigEvent;
import serverutils.serverlib.events.team.ForgeTeamConfigSavedEvent;
import serverutils.serverlib.events.team.ForgeTeamDataEvent;
import serverutils.serverlib.events.team.ForgeTeamOwnerChangedEvent;
import serverutils.serverlib.events.team.ForgeTeamPlayerJoinedEvent;
import serverutils.serverlib.events.team.ForgeTeamPlayerLeftEvent;
...
public class ForgeTeam extends FinalIDObject implements INBTSerializable<NBTTagCompound>, IConfigCallback
{
	...
	public IChatComponent getTitle()
	{
		...
		if (title.isEmpty())
		{
			cachedTitle = getOwner() != null ? getOwner().getDisplayName().appendText("'s Team") : new ChatComponentTranslation("serverlib.lang.team.no_team");
		}
		else
		{
	...
	public IChatComponent getCommandTitle()
	{
		...
		component.getChatStyle().setChatHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ChatComponentText("/team info " + getId())));
		component.getChatStyle().setChatClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/team info " + getId()));
		component.getChatStyle().setColor(getColor().getTextFormatting());
		return component;
	}
	...
	public ConfigGroup getSettings()
	{
		if (cachedConfig == null)
		{
			...
			cachedConfig.setDisplayName(new ChatComponentTranslation("gui.settings").appendSibling(StringUtils.bold(StringUtils.color(new ChatComponentText(" #" + toString()), EnumChatFormatting.DARK_GRAY), false)));
			ForgeTeamConfigEvent event = new ForgeTeamConfigEvent(this, cachedConfig);
			event.post();

			ConfigGroup main = cachedConfig.getGroup(ServerLib.MOD_ID);
			main.setDisplayName(new ChatComponentText(ServerLib.MOD_NAME));
	...
	{
		File dir = new File(universe.getWorldDirectory(), "data/server_lib/teams/");
