...
public class ItemIcon extends Icon {
	...
	private static class LazyItemIcon extends ItemIcon {
		...
		public ItemStack getStack() {
			if (!createdStack) {
				...
				if (ServerLibConfig.debugging.print_more_errors && stack == null) {
	...
	public static void drawItem3D(ItemStack stack) {
		...
		// IBakedModel bakedmodel = RenderItem.getInstance().getItemModelWithOverrides(stack, mc.theWorld, mc.thePlayer);
