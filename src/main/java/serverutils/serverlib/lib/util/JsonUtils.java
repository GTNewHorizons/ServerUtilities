...
import serverutils.serverlib.lib.util.text_components.TextComponentCountdown;
...
public class JsonUtils
{
	...
	public static JsonElement serializeTextComponent(@Nullable IChatComponent component)
	{
		...
		else if (component.getClass() == ChatComponentText.class && component.getSiblings().isEmpty() && component.getChatStyle().isEmpty())
		{
		...
		if (!component.getSiblings().isEmpty())
		{
			...
			for (Object text : component.getSiblings())
			{
				IChatComponent ichatcomponent = (IChatComponent) text;
		...
		else if (component instanceof ScoreObjective)
		{
			ScoreObjective score = (ScoreObjective) component;
			JsonObject json1 = new JsonObject();
			json1.addProperty("name", score.getName());
			json1.addProperty("objective", (Number) score.getCriteria());
			json1.addProperty("value", score.getDisplayName());
		...
		else
		{
			...
			if (json.has("text") || json.has("notification"))
			{
				...
				if (json.has("notification") || json.has("timer") || json.has("important"))
				{
					...
					{
						n.setImportant(net.minecraft.util.JsonUtils.getJsonElementBooleanValue(json, "important"));
					}
				}
			...
			else if (json.has("score"))
			{
				...
				component = new TextComponentScore(net.minecraft.util.JsonUtils.getJsonElementStringValue(o1, "name"), net.minecraft.util.JsonUtils.getJsonElementStringValue(o1, "objective"));

				if (o1.has("value"))
				{
					((TextComponentScore) component).setValue(net.minecraft.util.JsonUtils.getJsonElementStringValue(o1, "value"));
				}
			}
			else if (json.has("selector"))
			{
				component = new TextComponentSelector(net.minecraft.util.JsonUtils.getJsonElementStringValue(json, "selector"));
			}
			else if (json.has("keybind"))
			{
				component = new TextComponentKeybind(net.minecraft.util.JsonUtils.getJsonElementStringValue(json, "keybind"));
			}
			else
			{
				return null;
		...
		switch (nbt.getId())
		{
			case Constants.NBT.TAG_COMPOUND:
			{
				...
				if (nbt.isEmpty())
				{
					...
					for (String s : tagCompound.keySet())
					{
			...
			case Constants.NBT.TAG_BYTE:
				return new JsonPrimitive(((NBTBase.NBTPrimitive) nbt).func_150290_f());
			case Constants.NBT.TAG_SHORT:
				return new JsonPrimitive(((NBTBase.NBTPrimitive) nbt).func_150289_e());
			case Constants.NBT.TAG_INT:
				return new JsonPrimitive(((NBTBase.NBTPrimitive) nbt).func_150287_d());
			case Constants.NBT.TAG_LONG:
				return new JsonPrimitive(((NBTBase.NBTPrimitive) nbt).func_150291_c());
			case Constants.NBT.TAG_FLOAT:
				return new JsonPrimitive(((NBTBase.NBTPrimitive) nbt).func_150288_h());
