...
import serverutils.utils.net.MessageUpdateBadges;
...
public class ServerBadges {
    ...
    public static class ThreadReloadBadges extends Thread {
        ...
        public void run() {
            ...
            try {
                File file = LMFileUtils.newFile(new File(ServerUtilitiesLib.folderLocal, "badges.json"));
            ...
            ServerUtilities.logger
                    .info("Loaded " + map.size() + " badges in " + (LMUtils.millis() - msStarted) + " ms!");
            isDone = true;
        }
    }
