...
public class PermissionListPage // extends HTTPWebPage
{
    ...
    // allNodes.add(new NodeEntry(info.node, info.defaultValue, info.defaultOPValue,
    // desc.equals(info.node) ? "" : desc, null));
    // }
    //
    // allNodes.sort(null);
    //
    ...
    // variants.add(String.format("%s to %s", min == Integer.MIN_VALUE ? "-&infin;" : String.valueOf(min),
    ...
    //
    // variants.add(String.format("%s to %s",
    ...
    // row.td().addClass("center-text").attr("colspan", "2").span("", classOf(entry.player)).paired("code",
    // playerText);
    // } else {
    ...
    // }
    //
    // export.add(Arrays.asList(entry.getNode(), entry.player.getId(),
    // entry.player.getStringForGUI().getUnformattedText(),
    // entry.op.getStringForGUI().getUnformattedText(), variants2.toString()));
    // }
    //
    // try {
