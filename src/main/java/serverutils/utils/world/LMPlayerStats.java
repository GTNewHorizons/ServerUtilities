...
public class LMPlayerStats {
    ...
    public void getInfo(LMPlayerServer owner, List<IChatComponent> info, long ms) {
        if (lastSeen > 0L && !owner.isOnline())
            info.add(ServerUtilities.mod.chatComponent("label.last_seen", LMStringUtils.getTimeString(ms - lastSeen)));
        if (firstJoined > 0L)
            info.add(ServerUtilities.mod.chatComponent("label.joined", LMStringUtils.getTimeString(ms - firstJoined)));
        if (deaths > 0) info.add(ServerUtilities.mod.chatComponent("label.deaths", String.valueOf(deaths)));
